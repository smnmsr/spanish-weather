...
/**
 * Worker job to fetch extreme values (records) for a specific station.
 */
class FetchExtremeValuesJob implements ShouldQueue
{
    ...
    {
        try {
            Log::debug('FetchExtremeValuesJob: starting', [
                'batchId' => $this->weatherBatchId,
                'stationId' => $this->stationId,
            ]);

            $data = $aemet->getExtremeValues($this->stationId);

            $this->storeStationResult($data);
            $this->incrementCompletedCount();

            Log::info('FetchExtremeValuesJob: completed', [
                'batchId' => $this->weatherBatchId,
                'stationId' => $this->stationId,
            ]);
        } catch (\Exception $e) {
    ...
    protected function storeStationResult(array $data): void
    {
        ...
        if (! isset($results['extreme_values_by_station'])) {
            $results['extreme_values_by_station'] = [];
        }

        $results['extreme_values_by_station'][$this->stationId] = $data;
        Cache::put($resultsKey, $results, 24 * 60 * 60);
    }

    protected function recordJobFailure(\Exception $e): void
    {
        Log::error('FetchExtremeValuesJob failed', [
            'batchId' => $this->weatherBatchId,
            'stationId' => $this->stationId,
            'error' => $e->getMessage(),
        ]);
    ...
    protected function incrementCompletedCount(): void
    {
        ...
        Log::debug('FetchExtremeValuesJob: incremented completed count', [
            'batchId' => $this->weatherBatchId,
            'stationId' => $this->stationId,
