...
/**
 * Worker job to fetch daily climate values for a specific station.
 */
class FetchDailyValuesJob implements ShouldQueue
{
    ...
    {
        try {
            Log::debug('FetchDailyValuesJob: starting', [
                'batchId' => $this->weatherBatchId,
                'stationId' => $this->stationId,
            ]);

            $data = $aemet->getDailyClimateData($this->stationId, $this->startDate, $this->endDate);

            $this->storeStationResult($data);
            $this->incrementCompletedCount();

            Log::info('FetchDailyValuesJob: completed', [
                'batchId' => $this->weatherBatchId,
                'stationId' => $this->stationId,
                'count' => count($data),
            ]);
    ...
    protected function storeStationResult(array $data): void
    {
        ...
        if (! isset($results['daily_values_by_station'])) {
            $results['daily_values_by_station'] = [];
        }

        $results['daily_values_by_station'][$this->stationId] = $data;
        Cache::put($resultsKey, $results, 24 * 60 * 60);
    }

    protected function recordJobFailure(\Exception $e): void
    {
        Log::error('FetchDailyValuesJob failed', [
            'batchId' => $this->weatherBatchId,
            'stationId' => $this->stationId,
            'error' => $e->getMessage(),
        ]);
    ...
    protected function incrementCompletedCount(): void
    {
        ...
        Log::debug('FetchDailyValuesJob: incremented completed count', [
            'batchId' => $this->weatherBatchId,
            'stationId' => $this->stationId,
